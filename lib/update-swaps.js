...
function tenorFromText(text) {
  for (const tenor of WANTED_TENORS) {
    if (textMatchesTenor(text, tenor)) return tenor;
  }

  return null;
}

function extractRatesFromSebTableJson(json) {
  const rates = {};
    ...
    if (Array.isArray(node.headers) && Array.isArray(node.rows)) {
      ...
      if (maturityIndex >= 0 && priceIndex >= 0) {
        for (const row of node.rows) {
          ...
          const tenor = tenorFromText(cellValue(cells[maturityIndex]));
          if (!tenor || rates[tenor] !== undefined) continue;

          const value = parseNumber(cellValue(cells[priceIndex]));
...
function extractRatesFromApiJson(json, currency) {
  const rates = extractRatesFromSebTableJson(json);
